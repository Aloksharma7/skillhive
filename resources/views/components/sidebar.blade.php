    ...
    <div id="sidebar" class="fixed lg:static inset-y-0 left-0 z-40 w-64 bg-gradient-to-b from-[#4F46E5] to-[#4338CA] text-white shadow-2xl flex flex-col transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out">
        ...
        <nav class="flex-1 p-4 overflow-y-auto">
            <div class="space-y-1">
                ...
                <!-- Blogs -->
                <a href="{{ route('blogs') }}"
                   class="nav-link flex items-center space-x-3 px-3 py-2.5 rounded-lg transition-all duration-200 {{ request()->routeIs('blogs') ? 'bg-white/20 text-white shadow-lg' : 'hover:bg-white/10 text-white/80 hover:text-white' }}">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @auth
                    @if (Auth::user()->role === 'superadmin')
                        ...
                        <a href="{{ route('pdfresources.index') }}"
